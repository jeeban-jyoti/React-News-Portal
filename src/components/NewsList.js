...
const NewsList = () => {
    ...
    const handleSearch = (e) => {
        ...
        dispatch(fetchEventRegistryArticles({ query, page: 1, category: currentCategory }));
    };
    ...
    const handlePageChange = (page) => {
        ...
        dispatch(fetchEventRegistryArticles({ query: currentQuery, page, category: currentCategory }));
    };

    const handleCategorySelect = (category) => {
        dispatch(setCategory(category));
        setShowDropdown(false);
        dispatch(fetchEventRegistryArticles({ query: currentQuery, page: 1, category }));
    };
    ...
    return (
        <div className="news-list-container">
            ...
            <div className="toolbar">
                <button className='favToggler' onClick={() => dispatch(toggleFavoriteView())}>
                    {isFav ? 'Show All Articles' : 'Show Favorites'}
                </button>
                <button className='categoryDropdown' onClick={() => setShowDropdown(!showDropdown)}>
                    Select Category
                </button>
                {showDropdown && (
                    <div className="dropdown-menu">
                        <button onClick={() => handleCategorySelect('all')}>All</button>
                        <button onClick={() => handleCategorySelect('business')}>Business</button>
                        <button onClick={() => handleCategorySelect('technology')}>Technology</button>
                        <button onClick={() => handleCategorySelect('entertainment')}>Entertainment</button>
                        <button onClick={() => handleCategorySelect('politics')}>Politics</button>
